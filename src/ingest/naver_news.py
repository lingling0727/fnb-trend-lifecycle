...
def collect_news_year(keyword: str, year: int) -> pd.DataFrame:
    headers = {
...
if __name__ == "__main__":
    ...
    for i, kw in enumerate(keywords):
        ...
        for year in YEARS:
            out_path = f"data/raw/news/{kw}_{year}.csv"
            if os.path.exists(out_path):
                kw_dfs.append(pd.read_csv(out_path))
                continue
            df = collect_news_year(kw, year)
            if not df.empty:
        ...
        if kw_dfs:
            ...
            kw_combined.to_csv(f"data/raw/news/{kw}.csv", index=False, encoding="utf-8-sig")
            all_dfs.append(kw_combined)
            print(f"  → 합계 {len(kw_combined)}건")

    if all_dfs:
        combined = pd.concat(all_dfs, ignore_index=True)
        combined.to_csv("data/raw/news/all_keywords.csv", index=False, encoding="utf-8-sig")
        print(f"\n완료: 총 {len(combined)}건 → data/raw/news/")
