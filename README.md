...
후보 키워드는 연구자의 사전 지식과 온라인 미디어 모니터링(유튜브 숏폼·커뮤니티·백과사전류)을 통해 직접 구성했으며, 트렌드 리포트(배달의민족·aT 등)는 F&B 유행의 유형·카테고리 참고에 활용했다. 이후 네이버 DataLab 검색량으로 화제성을 확인해 최종 38개를 선정했다. 분석 기준 시점은 2016년(네이버 DataLab 서비스 시작)으로 통일했다. 키워드 선정이 연구자 판단에 기반한 목적표집(선정 편향 존재)이라는 점은 한계에 명시한다.
